...
exports.scheduleAssignment = async (req, res) => {
  //file
  const { batchId, date, time,name,path,istDateTime } = req.body;
  let job={};
  job.batchId = batchId;
  ...
  try {
    ...
    batch.assigned.push({path:path,date:date,time:time,istDateTime:istDateTime,name:name});
    await batch.save();
    res.json("SAB THIK");
  } catch (error) {
...
cron.schedule('* * * * *', async () => {
    ...
    await Promise.all(curJobs.map(async (item)=>{
        ...
        let newAssigned = batch.assigned.map(async (itm)=>{
            ...
            {   
                let assignment={path:itm.path,istDateTime:itm.istDateTime,responses:[],batchId:bId,name:itm.name}
